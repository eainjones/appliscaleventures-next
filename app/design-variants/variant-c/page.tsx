...
import { Sparkles, Rocket, Code2, Database } from 'lucide-react';
...
export default function VariantCPage() {
  return (
    <div className="min-h-screen bg-slate-950 text-white relative overflow-hidden">
      ...
      <header className="fixed top-0 w-full bg-slate-900/50 backdrop-blur-2xl border-b border-white/10 z-50">
        <div className="max-w-7xl mx-auto px-8 py-4 flex justify-between items-center">
          <Link href="/design-variants" className="text-sm font-medium text-cyan-400 hover:text-cyan-300 transition-colors">
            ← Back to Variants
          </Link>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-cyan-400 rounded-full animate-pulse"></div>
            <span className="text-xs text-white/60">Variant C: Tech-Forward</span>
          </div>
        </div>
      </header>
      ...
      <section className="py-24 px-8 relative">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-12">
            ...
            <button className="hidden md:block px-6 py-3 bg-cyan-500/20 border border-cyan-500/30 rounded-xl font-medium text-cyan-300 hover:bg-cyan-500/30 transition-all">
              View All →
            </button>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {['demoboost', 'quick9', 'harmonyze', 'TVAD.ia', 'Bedrock', 'Erebor', 'First Party', '+ More'].map((company, i) => (
              <div
                key={i}
                className={`group relative p-8 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl hover:bg-white/10 hover:border-cyan-500/30 transition-all cursor-pointer ${
                  i === 0 ? 'md:col-span-2' : ''
                } ${i === 7 ? 'md:col-span-3 bg-gradient-to-r from-cyan-500/10 to-blue-500/10' : ''}`}
              >
      ...
      {/* Footer */}
      <footer className="py-12 px-8 border-t border-white/10 relative">
        <div className="max-w-7xl mx-auto text-center text-slate-500 text-sm">
          Variant C: Tech-Forward • Appliscale Ventures
        </div>
      </footer>
    </div>
  );
